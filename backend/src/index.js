...
if (isProduction) {
  if (!process.env.JWT_SECRET || process.env.JWT_SECRET.trim().length < 16) {
    throw new Error(
      "Production requires JWT_SECRET (min 16 chars). Set in Vercel Dashboard → Environment Variables.",
    );
  }
  if (
    ...
    throw new Error(
      "Production requires DATABASE_URL (Neon Postgres). Set in Vercel Dashboard → Environment Variables.",
    );
  }
}
...
app.get("/api/health", async (req, res) => {
  ...
  if (
    ...
    payload.hint =
      "Set DATABASE_URL and JWT_SECRET in your host environment. See backend/RENDER_DEPLOY.md for deployment setup.";
  }
...
const simpleInventoryRoutes = require("./routes/simpleInventory");
const simpleReferralsRoutes = require("./routes/simpleReferrals");
const simpleFollowUpsRoutes = require("./routes/simpleFollowUps");
const simplePaymentsRoutes = require("./routes/simplePayments");
const simpleStockRoutes = require("./routes/simpleStock");
const simpleRemoteConfigRoutes = require("./routes/simpleRemoteConfig");
...
app.use("/api/simple-referrals", simpleReferralsRoutes);
app.use("/api/simple-followups", simpleFollowUpsRoutes);
app.use("/api/simple-payments", simplePaymentsRoutes);
app.use("/api/simple-stock", simpleStockRoutes);
app.use("/api/remote-config", simpleRemoteConfigRoutes);
...
📱 For physical device: use your machine IP, e.g. http://YOUR_IP:${port}/api
  `);
      ...
      const forceSqliteStartup =
        process.env.USE_SQLITE &&
        ["true", "1", "yes"].includes(
          String(process.env.USE_SQLITE).toLowerCase(),
        );
      const usingPostgresStartup =
        !forceSqliteStartup &&
        !!(
          process.env.DATABASE_URL &&
          process.env.DATABASE_URL.trim() &&
      ...
      console.log(
        usingPostgresStartup
          ? "📦 Using Postgres (DATABASE_URL set)"
          : "📦 Using SQLite (no DATABASE_URL or USE_SQLITE=true)",
      );
      try {
        ...
        if (usingPostgresStartup) {
          try {
            ...
            console.error(
              "⚠️ Init-db run failed (tables may already exist):",
              initErr.message,
            );
          }
        }
