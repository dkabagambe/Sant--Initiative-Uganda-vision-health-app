...
router.patch("/:id/stock", authenticate, async (req, res) => {
  try {
    ...
    const { stock_quantity, stock_standard, stock_metal, stock_fashion } =
      req.body;
    ...
    const existing =
      await sql`SELECT id FROM vht_stock WHERE health_worker_id = ${healthWorkerId} AND product_id = ${id}`;
...
router.get("/stats", authenticate, async (req, res) => {
  try {
    ...
    const hasVhtStockRes =
      await sql`SELECT COUNT(*) AS cnt FROM vht_stock WHERE health_worker_id = ${healthWorkerId}`;
