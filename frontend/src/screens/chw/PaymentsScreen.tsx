...
export default function PaymentsScreen() {
  ...
  const [providerFilter, setProviderFilter] = useState<"all" | "mtn" | "airtel">("all");
  ...
  // ── Record payment form ───────────────────────────────────────────────────
  const [recordPaymentMethod, setRecordPaymentMethod] = useState<"cash" | "mobile_money">("cash");
  ...
  const submitRecordedPayment = async () => {
    ...
    if (!recordClientPhone.trim() || !isValidUgMobile(recordClientPhone.trim())) {
      Alert.alert("Validation", "Enter a valid Uganda mobile number (e.g. 0773445535).");
      return;
    }
    ...
    try {
      const result = await apiService.createPayment({
        ...
        paymentType: "full",
        ...(recordPaymentMethod === "mobile_money" && { provider: recordProvider }),
      });
  ...
  const confirmMarkAsPaid = async () => {
    ...
    try {
      const result = await apiService.updatePaymentStatus(selectedPayment.id, "completed");
      ...
      Alert.alert("Error", error?.message || "Failed to update payment. Please try again.");
    } finally {
  ...
  const getNextPendingDueDate = (clientPhone: string) => {
    const pending = payments
      .filter((p) => p.client_phone === clientPhone && (p.status === "pending" || p.status === "overdue") && p.due_date)
  ...
  const filteredPayments = payments.filter((p) => {
    ...
    const matchesProvider =
      providerFilter === "all" || (p.provider || "").toLowerCase() === providerFilter;
    const matchesMethod =
      methodFilter === "all" || (p.payment_method || "").toLowerCase() === methodFilter;
  ...
  const pendingAmount   = payments.filter((p) => p.status === "pending" || p.status === "overdue")
    .reduce((s, p) => s + (parseFloat(p.amount) || 0), 0);
  const collectedAmount = payments.filter((p) => p.status === "completed")
    .reduce((s, p) => s + (parseFloat(p.amount) || 0), 0);
  ...
  const monthlyCollected = monthlyPayments.filter((p) => p.status === "completed")
    .reduce((s, p) => s + (Number(p.amount) || 0), 0);
  const monthlyRemaining = Math.max(monthlyExpected - monthlyCollected, 0);
  const monthlyPercent   = monthlyExpected > 0 ? (monthlyCollected / monthlyExpected) * 100 : 0;
  const monthlyPeriod    = now.toLocaleDateString("en-US", { month: "long", year: "numeric" });
  ...
  const handleExport = async () => {
    try {
      await exportCsvFile({
        ...
        headers: ["Client", "Phone", "Status", "Method", "Amount (UGX)", "Due Date", "Transaction ID", "Date"],
      ...
      >
        <View style={styles.paymentHeader}>
          <View style={styles.clientInfo}>
            ...
            <View style={styles.clientDetails}>
              <Text style={styles.clientName}>{payment.client_name || "Unknown Client"}</Text>
              <Text style={styles.phoneNumber}>{payment.client_phone || "—"}</Text>
              {payment.product_name ? (
                <Text style={styles.productInfo}>
                  {payment.product_name}{payment.product_power ? ` (${payment.product_power})` : ""}
                </Text>
              ) : null}
        ...
        <View style={styles.paymentFooter}>
          <View style={styles.statusContainer}>
            <View style={[
              styles.statusDot,
              isOverdue ? styles.overdueDot : isPending ? styles.pendingDot : styles.completedDot,
            ]} />
            <Text style={isOverdue ? styles.overdueText : isPending ? styles.pendingText : styles.completedText}>
    ...
    <SafeAreaView style={styles.container}>
      ...
      >
        ...
        <View style={styles.summaryRow}>
          <TouchableOpacity
            style={[styles.summaryCard, activeTab === "pending" && !overdueOnly && styles.summaryCardActive]}
            onPress={() => { setActiveTab("pending"); setOverdueOnly(false); }}
            activeOpacity={0.8}
          >
            ...
            <Text style={styles.summaryAmount}>UGX {pendingAmount.toLocaleString()}</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={[styles.summaryCard, overdueOnly && styles.summaryCardOverdue]}
            onPress={() => { setActiveTab("pending"); setOverdueOnly(true); }}
            activeOpacity={0.8}
          >
            <View style={[styles.summaryIcon, { backgroundColor: "#FEE2E2" }]}>
              <FontAwesome5 name="exclamation-triangle" size={18} color="#DC2626" />
            </View>
            <Text style={[styles.summaryNumber, { color: "#DC2626" }]}>{overdueCount}</Text>
            <Text style={styles.summaryLabel}>Overdue</Text>
            <Text style={[styles.summaryAmount, { color: "#DC2626" }]}>
              UGX {payments.filter(p => p.status === "overdue").reduce((s,p) => s + (parseFloat(p.amount)||0), 0).toLocaleString()}
            </Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={[styles.summaryCard, activeTab === "completed" && styles.summaryCardCompleted]}
            onPress={() => { setActiveTab("completed"); setOverdueOnly(false); }}
            activeOpacity={0.8}
          >
            ...
            <Text style={[styles.summaryAmount, { color: "#059669" }]}>UGX {collectedAmount.toLocaleString()}</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={styles.tabsContainer}>
          ...
          >
            <Text style={[styles.tabText, activeTab === "pending" && styles.activeTabText]}>
              {overdueOnly ? `Overdue (${overdueCount})` : `Pending (${pendingCount})`}
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
          ...
          {filteredPayments.length === 0 ? (
            <View style={styles.emptyState}>
              ...
              <Text style={styles.emptyTitle}>
                {searchQuery ? "No results found" : `No ${overdueOnly ? "overdue" : activeTab} payments`}
              </Text>
              <Text style={styles.emptySubtitle}>
                {searchQuery ? "Try a different search" : activeTab === "pending" ? "All payments are up to date" : "No completed payments yet"}
              </Text>
              {activeTab === "pending" && !searchQuery && (
                <TouchableOpacity style={styles.emptyAction} onPress={handleRecordPayment}>
        ...
        <View style={styles.monthlySummary}>
          ...
          <View style={styles.monthlyRow}>
            <View style={styles.monthlyItem}>
              ...
              <Text style={styles.monthlyValue}>UGX {monthlyExpected.toLocaleString()}</Text>
            </View>
            <View style={styles.monthlyItem}>
              <Text style={styles.monthlyLabel}>Collected</Text>
              <Text style={[styles.monthlyValue, { color: "#059669" }]}>UGX {monthlyCollected.toLocaleString()}</Text>
            </View>
            <View style={styles.monthlyItem}>
              <Text style={styles.monthlyLabel}>Remaining</Text>
              <Text style={[styles.monthlyValue, { color: "#DC2626" }]}>UGX {monthlyRemaining.toLocaleString()}</Text>
            </View>
          </View>
          <View style={styles.progressBg}>
            <View style={[styles.progressFill, {
              width: `${Math.min(monthlyPercent, 100)}%`,
              backgroundColor: monthlyCollected >= monthlyExpected && monthlyExpected > 0 ? "#059669" : "#F59E0B",
            }]} />
          </View>
          <Text style={styles.progressLabel}>{monthlyPercent.toFixed(0)}% collected</Text>
        </View>

        {/* ── Quick Actions ─────────────────────────────────────────────── */}
        <View style={styles.quickActions}>
          <TouchableOpacity style={styles.actionBtn} onPress={handleRecordPayment}>
      ...
      <Modal animationType="slide" transparent visible={recordModalVisible} onRequestClose={() => setRecordModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            <View style={styles.modalHandle} />
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Record Payment</Text>
              <TouchableOpacity onPress={() => !submittingRecord && setRecordModalVisible(false)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
            ...
            <View style={styles.methodRow}>
              {(["cash", "mobile_money"] as const).map((m) => (
                <TouchableOpacity
                  key={m}
                  style={[styles.methodBtn, recordPaymentMethod === m && styles.methodBtnActive]}
                ...
                >
                  <Ionicons
                    ...
                    style={{ marginRight: 4 }}
                  />
                  <Text style={[styles.methodBtnText, recordPaymentMethod === m && styles.methodBtnTextActive]}>
              ...
              <>
                ...
                <View style={styles.methodRow}>
                  {(["mtn", "airtel"] as const).map((p) => (
                    <TouchableOpacity
                      key={p}
                      style={[styles.methodBtn, recordProvider === p && styles.methodBtnActive]}
                      onPress={() => setRecordProvider(p)}
                    >
                      <Text style={[styles.methodBtnText, recordProvider === p && styles.methodBtnTextActive]}>
                        {p.toUpperCase()}
                      </Text>
                    </TouchableOpacity>
                  ))}
                ...
                <Text style={styles.hintText}>
                  Payment will be recorded as pending. Mark as paid after the client confirms.
                </Text>
              </>
            )}
            ...
            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.cancelBtn} onPress={() => setRecordModalVisible(false)} disabled={submittingRecord}>
                <Text style={styles.cancelBtnText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[styles.confirmBtn, submittingRecord && { opacity: 0.7 }]} onPress={submitRecordedPayment} disabled={submittingRecord}>
                {submittingRecord
      ...
      {/* ── Mark as Paid Modal ────────────────────────────────────────────── */}
      <Modal animationType="slide" transparent visible={markPaidModalVisible} onRequestClose={() => !confirmingPayment && setMarkPaidModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalSheet}>
            <View style={styles.modalHandle} />
            <View style={styles.modalHeader}>
              <Text style={styles.modalTitle}>Confirm Payment</Text>
              <TouchableOpacity onPress={() => !confirmingPayment && setMarkPaidModalVisible(false)}>
                <Ionicons name="close" size={24} color="#374151" />
              </TouchableOpacity>
            </View>
            ...
            {selectedPayment && (
              <>
                <View style={styles.confirmInfo}>
                  ...
                  <Text style={styles.confirmName}>{selectedPayment.client_name || "Client"}</Text>
                  <Text style={styles.confirmPhone}>{selectedPayment.client_phone || "—"}</Text>
                  <View style={styles.confirmAmountBox}>
                    <Text style={styles.confirmAmountLabel}>Amount to confirm</Text>
                  ...
                  {selectedPayment.status === "overdue" && (
                    <View style={styles.overdueWarning}>
                      ...
                      <Text style={styles.overdueWarningText}>This payment is overdue</Text>
                    </View>
                  )}
                </View>
                <View style={styles.modalActions}>
                  <TouchableOpacity style={styles.cancelBtn} onPress={() => setMarkPaidModalVisible(false)} disabled={confirmingPayment}>
                    <Text style={styles.cancelBtnText}>Cancel</Text>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.confirmBtn, { backgroundColor: "#059669" }, confirmingPayment && { opacity: 0.7 }]} onPress={confirmMarkAsPaid} disabled={confirmingPayment}>
                    {confirmingPayment
                      ? <ActivityIndicator size="small" color="#FFFFFF" />
                      : <>
          ...
          <View style={styles.modalSheet}>
            ...
            <View style={styles.methodRow}>
              {(["all", "mtn", "airtel"] as const).map((p) => (
                <TouchableOpacity key={p} style={[styles.methodBtn, providerFilter === p && styles.methodBtnActive]} onPress={() => setProviderFilter(p)}>
                  <Text style={[styles.methodBtnText, providerFilter === p && styles.methodBtnTextActive]}>{p.toUpperCase()}</Text>
                </TouchableOpacity>
              ))}
            </View>
            ...
            <View style={styles.methodRow}>
              {([["all","ALL"],["cash","Cash"],["mobile_money","Mobile Money"]] as const).map(([val, label]) => (
                <TouchableOpacity key={val} style={[styles.methodBtn, methodFilter === val && styles.methodBtnActive]} onPress={() => setMethodFilter(val)}>
                  <Text style={[styles.methodBtnText, methodFilter === val && styles.methodBtnTextActive]}>{label}</Text>
                </TouchableOpacity>
              ))}
            </View>

            <View style={styles.modalActions}>
              <TouchableOpacity style={styles.cancelBtn} onPress={() => { setProviderFilter("all"); setMethodFilter("all"); setOverdueOnly(false); }}>
                <Text style={styles.cancelBtnText}>Reset</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.confirmBtn} onPress={() => setFilterModalVisible(false)}>
      ...
      <Modal animationType="slide" transparent visible={clientSummaryVisible} onRequestClose={() => setClientSummaryVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={[styles.modalSheet, { maxHeight: "75%" }]}>
            ...
            <View style={styles.clientStatsRow}>
              <View style={styles.clientStat}>
                <Text style={styles.clientStatNum}>{selectedClientPayments.length}</Text>
                <Text style={styles.clientStatLabel}>Total</Text>
              </View>
              <View style={styles.clientStat}>
                <Text style={[styles.clientStatNum, { color: "#059669" }]}>
                  {selectedClientPayments.filter(p => p.status === "completed").length}
                </Text>
                <Text style={styles.clientStatLabel}>Paid</Text>
              </View>
              <View style={styles.clientStat}>
                <Text style={[styles.clientStatNum, { color: "#F59E0B" }]}>
                  {selectedClientPayments.filter(p => p.status === "pending" || p.status === "overdue").length}
                </Text>
                <Text style={styles.clientStatLabel}>Pending</Text>
              </View>
              <View style={styles.clientStat}>
                <Text style={styles.clientStatNum}>
                  {(selectedClientPayments.filter(p => p.status === "completed").reduce((s,p) => s + (parseFloat(p.amount)||0), 0) / 1000).toFixed(0)}K
                </Text>
                ...
                <View key={p.id} style={styles.historyItem}>
                  <View style={{ flex: 1 }}>
                    <Text style={styles.historyTitle}>
                      ...
                      {p.payment_type === "installment" && p.installment_number ? ` • Installment ${p.installment_number}/${p.total_installments}` : ""}
                    </Text>
                    <Text style={styles.historyDate}>
                      {new Date(p.payment_date || p.created_at || 0).toLocaleDateString("en-US", { day: "numeric", month: "short", year: "numeric" })}
                      {p.due_date ? ` • Due: ${new Date(p.due_date).toLocaleDateString("en-US", { day: "numeric", month: "short" })}` : ""}
                    </Text>
                  </View>
                  <View style={[
                    styles.statusPill,
                    p.status === "completed" ? styles.pillCompleted : p.status === "overdue" ? styles.pillOverdue : styles.pillPending,
                  ]}>
                    <Text style={[
                      styles.statusPillText,
                      p.status === "completed" ? { color: "#059669" } : p.status === "overdue" ? { color: "#DC2626" } : { color: "#F59E0B" },
                    ]}>
                      {p.status}
                    </Text>
                  </View>
      ...
      <View style={[styles.bottomNav, { paddingBottom: Math.max(insets.bottom, 12) }]}>
        <TouchableOpacity style={styles.navItem} onPress={() => navigation.navigate("CHWDashboard")}>
          <Ionicons name="home-outline" size={22} color="#6B7280" />
          <Text style={styles.navText}>Home</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.navItem} onPress={() => navigation.navigate("Screen" as any, { screen: "VHTScreeningStep1" } as any)}>
          <Ionicons name="eye-outline" size={22} color="#6B7280" />
          <Text style={styles.navText}>Screen</Text>
        </TouchableOpacity>
        <TouchableOpacity style={styles.navItem} onPress={() => navigation.navigate("InventoryScreen")}>
        ...
        <TouchableOpacity style={styles.navItem} onPress={() => navigation.navigate("ReferralsScreen")}>
...
const styles = StyleSheet.create({
  ...
  summaryIcon: { width: 36, height: 36, borderRadius: 18, justifyContent: "center", alignItems: "center", marginBottom: 6 },
  summaryNumber: { fontSize: 22, fontWeight: "700", color: "#1F2937" },
  summaryLabel: { fontSize: 11, color: "#6B7280", marginTop: 1 },
  summaryAmount: { fontSize: 10, color: "#6B7280", marginTop: 2, textAlign: "center" },
  ...
  recordBtn: {
    ...
    shadowColor: "#1E40AF", shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.3, shadowRadius: 8, elevation: 4,
  },
  ...
  overdueBadge: {
    ...
    backgroundColor: "#FECACA", paddingHorizontal: 5, paddingVertical: 2,
  ...
  completedBadge: {
    flexDirection: "row", alignItems: "center",
    backgroundColor: "#D1FAE5", paddingHorizontal: 10, paddingVertical: 5, borderRadius: 6, gap: 4,
  },
  completedBadgeText: { fontSize: 13, color: "#059669", fontWeight: "700" },

  // Empty state
  emptyState: { alignItems: "center", paddingVertical: 40 },
  emptyTitle: { fontSize: 17, fontWeight: "600", color: "#374151", marginTop: 14, marginBottom: 6 },
  ...
  monthlyHeader: { flexDirection: "row", justifyContent: "space-between", alignItems: "center", marginBottom: 14 },
  monthlyTitle: { fontSize: 16, fontWeight: "700", color: "#1F2937" },
  monthlyPeriod: { fontSize: 13, color: "#6B7280" },
  monthlyRow: { flexDirection: "row", justifyContent: "space-around", marginBottom: 14 },
  ...
  // Quick actions
  quickActions: { flexDirection: "row", justifyContent: "space-around", marginBottom: 8 },
  actionBtn: { alignItems: "center", padding: 12 },
  actionBtnText: { fontSize: 11, color: "#2563EB", marginTop: 4, fontWeight: "500" },
  ...
  hintText: { fontSize: 12, color: "#6B7280", marginTop: 6, marginBottom: 4, fontStyle: "italic" },
  modalActions: { flexDirection: "row", gap: 12, marginTop: 20 },
  cancelBtn: {
    flex: 1, paddingVertical: 14, borderWidth: 1.5, borderColor: "#D1D5DB",
    borderRadius: 10, alignItems: "center",
  },
  cancelBtnText: { fontSize: 15, fontWeight: "600", color: "#374151" },
  confirmBtn: {
    flex: 1, paddingVertical: 14, backgroundColor: "#1E40AF",
  ...
  confirmName: { fontSize: 18, fontWeight: "700", color: "#1F2937", marginTop: 10, marginBottom: 4 },
  confirmPhone: { fontSize: 14, color: "#6B7280", marginBottom: 12 },
  confirmAmountBox: {
    backgroundColor: "#F0FDF4", borderRadius: 12, paddingVertical: 12,
    paddingHorizontal: 24, alignItems: "center", marginBottom: 8,
  },
  confirmAmountLabel: { fontSize: 12, color: "#6B7280" },
  confirmAmount: { fontSize: 26, fontWeight: "700", color: "#059669" },
  overdueWarning: {
    flexDirection: "row", alignItems: "center", gap: 6,
    backgroundColor: "#FEE2E2", paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, marginTop: 8,
  },
  overdueWarningText: { color: "#DC2626", fontSize: 13, fontWeight: "600" },

  // Client summary modal
  clientStatsRow: { flexDirection: "row", justifyContent: "space-around", marginTop: 12, marginBottom: 4 },
  ...
  bottomNav: {
    ...
    shadowColor: "#000", shadowOffset: { width: 0, height: -2 }, shadowOpacity: 0.08, shadowRadius: 4, elevation: 10,
  },
  ...
  activeNavIcon: {
    ...
    justifyContent: "center", alignItems: "center", marginTop: -18, marginBottom: 2,
  },
